...
function getJoinedTables_(datas, ArraySql)
{
  // ArraySql = "ColN"
  var result = [];
  var index = ArraySql.match(/\d+$/gm) * 1;
  
  return joinArrays_(index, datas);
...
function join2Arrays_(index, arr1, arr2)
{
  ...
  var newKeys = arr2.map(returnKey_); // keys from array arr2
  ...
  var newRow = [];
  var newPos = -1;
  var ll = arr1[0].length; // number of columns
  
  var indexsDelete = {};
  for (var i = 0, l = arr1.length; i < l; i++)        
  {
    row = arr1[i]; // existing row, from arr1
    key = row[index - 1]; // existing key, from arr1
    
    newPos = newKeys.indexOf(key); // if key is present in arr2, return the position
        
    if (newPos > -1)
    {
      newRow = arr2[newPos]; // replaces row with new one
      result.push(newRow);
      // delete element from array
      indexsDelete['' + newPos] = 'ok';
    }
    else { result.push(row); } // add new row  
  }
